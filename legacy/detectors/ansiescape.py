...
class Escaped(StringDetector):
    ...
    def __init__(self, config_root=_config):
        base_seqs = checkmate.resources.ansi.ESCAPED_SEQS
        commands = checkmate.resources.ansi.ESCAPED_COMMANDS
        suffixes = set(["[", "]"])
...
class Raw(StringDetector):
    ...
    def __init__(self, config_root=_config):
        base_seqs = checkmate.resources.ansi.RAW_SEQS
        commands = checkmate.resources.ansi.RAW_COMMANDS
        suffixes = set(["[", "]"])
        all_substrings = [f"{x}{y}" for x in commands for y in suffixes] + base_seqs
        super().__init__(all_substrings, config_root)
