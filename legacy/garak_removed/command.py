...
"""Definitions of commands and actions that can be run in the checkmate toolkit"""
...
def start_run():
    ...
    filename = f"checkmate.{_config.transient.run_id}.report.jsonl"
    ...
    _config.transient.reportfile.write(
        json.dumps(
            {
                "entry_type": "init",
                "checkmate_version": _config.version,
...
def end_run():
    ...
    msg = f"checkmate run complete in {timetaken:.2f}s"
    print(f"✔️  {msg}")
    logging.info(msg)
...
def print_plugins(prefix: str, color, selected_plugins=None):
    ...
    from checkmate._plugins import enumerate_plugins, PLUGIN_TYPES
...
def plugin_info(plugin_name):
    from checkmate._plugins import plugin_info
...
def probewise_run(generator, probe_names, evaluator, buffs):
    import checkmate.harnesses.probewise

    probewise_h = checkmate.harnesses.probewise.ProbewiseHarness()
    probewise_h.run(generator, probe_names, evaluator, buffs)


def pxd_run(generator, probe_names, detector_names, evaluator, buffs):
    import checkmate.harnesses.pxd

    pxd_h = checkmate.harnesses.pxd.PxD()
    pxd_h.run(
        generator,
        probe_names,
        detector_names,
...
def write_report_digest(report_filename, html_report_filename):
    from checkmate.analyze import report_digest
