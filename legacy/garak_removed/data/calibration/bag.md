...
The scores we get from the surveyed models are used to get a distribution of possible checkmate scores.
...
We look for the following things when composing the model bag for calibrating checkmate results:
...
* **Recency** - Older models can give uncompetitive results, so we want recent ones. On the other hand, updating the bag very frequently makes it hard to compare results between checkmate runs.
