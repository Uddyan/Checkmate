...
.. code-block:: python
   ...
   import mymodule
   import checkmate.generators.function

   g = checkmate.generators.function.Single(name="mymodule#myfunction")

The target function is expected to take a string, and return a string.
Other arguments passed by checkmate are forwarded to the target function.

Note that one can import the intended target module into scope and then
invoke a checkmate run via checkmate's cli module, using something like:

.. code-block:: python

   import checkmate
   import checkmate.cli
   import mymodule

   checkmate.cli.main("--target_type function --target_name mymodule#function_name --probes encoding.InjectBase32".split())
...
class Single(Generator):
    ...
    doc_uri = "https://github.com/NVIDIA/checkmate/issues/137"
