...
from checkmate.attempt import Message, Turn, Conversation
from checkmate.exception import GarakException
from checkmate.generators.openai import OpenAICompatible
...
class NVOpenAIChat(OpenAICompatible):
    ...
    #. Run checkmate, setting ``--target_type 'nim.NVIDIAOpenAIChat'`` and ``--target_name`` to
...
class NVMultimodal(NVOpenAIChat):
    ...
    #. In your console, set the ``NIM_API_KEY`` variable to this API key.
       ...
       On Linux, this might look like ``export NIM_API_KEY="nvapi-xXxXxXx"``.
    #. Run checkmate, setting ``--target_type 'nim.NVMultimodal'`` and ``--target_name`` to
