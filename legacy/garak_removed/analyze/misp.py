...
"""
report on & validate categories maintained within checkmate
...
def misp_report(include_untagged: bool = True) -> None:
    ...
    for plugin_name, active in _plugins.enumerate_plugins("probes"):
        ...
        m = importlib.import_module(f"checkmate.{module_name}")
        ...
        for tag in tags:
            if tag not in tag_descriptions:
                print(f"{plugin_name}: tag {tag} undefined in checkmate/data/tags.misp.tsv")
...
def main(argv=None) -> None:
    ...
    parser = argparse.ArgumentParser(
        prog="python -m checkmate.analyze.misp",
