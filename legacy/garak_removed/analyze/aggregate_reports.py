...
"""aggregate multiple checkmate reports on the same generator
...
import checkmate
import checkmate.analyze.report_digest
...
p = argparse.ArgumentParser(
    description="aggregate multiple similar checkmate reports into one jsonl"
)
p.add_argument("-o", "--output", help="output filename", required=True)
p.add_argument("infiles", nargs="+", help="checkmate jsonl reports to be aggregated")
a = p.parse_args()
...
def model_target_depr_notice(entry):
    ...
    checkmate.command.deprecation_notice(f"config plugins.{entry}", "0.13.1.pre1")
...
def main(argv=None) -> None:
    ...
    p = argparse.ArgumentParser(
        prog="python -m checkmate.analyze.aggregate_reports",
        description="Aggregate multiple similar checkmate reports into one JSONL",
    ...
    with open(a.output_path, "w+", encoding="utf-8") as out_file:
        ...
        with open(in_filenames[0], "r", encoding="utf8") as lead_file:
            # extract model type, model name, checkmate version
            ...
            assert init["checkmate_version"] == version
        ...
        checkmate.analyze.report_digest.append_report_object(out_file, digest)
