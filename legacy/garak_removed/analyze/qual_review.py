...
# generate a qualitative review of a checkmate report
...
from checkmate.probes import Tier

PROBE_DETECTOR_SEP = "+"


def build_tiers() -> dict:
    from checkmate._plugins import enumerate_plugins, plugin_info
...
def qual_review(report_path: str) -> None:
    ...
    for probe_detector in probe_detector_scores.keys():
        if probe_detector.split(PROBE_DETECTOR_SEP)[0] in t1_probe_names:
            ...
            if passrate < checkmate.analyze.ABSOLUTE_DEFCON_BOUNDS.BELOW_AVG or (
                z is not None and z < checkmate.analyze.RELATIVE_DEFCON_BOUNDS.BELOW_AVG
            ):
                print("Issues found:")
                if passrate < checkmate.analyze.ABSOLUTE_DEFCON_BOUNDS.BELOW_AVG:
                    print(f"* low pass rate `{passrate:0.4f}`")
                if z is not None and z < checkmate.analyze.RELATIVE_DEFCON_BOUNDS.BELOW_AVG:
...
def main(argv=None) -> None:
    ...
    checkmate._config.load_config()
    print(
        f"checkmate {checkmate.__description__} v{checkmate._config.version} ( https://github.com/NVIDIA/checkmate )"
    )

    parser = argparse.ArgumentParser(
        prog="python -m checkmate.analyze.qual_review",
    ...
    parser.add_argument(
        ...
        "--report_path",
        required=False,
        help="Path to the checkmate JSONL report",
    )
    parser.add_argument(
        "report_path_positional",
        nargs="?",
        help="Path to the checkmate JSONL report (positional)",
    )
