...
import checkmate.resources.ansi
...
class AnsiEscaped(checkmate.probes.Probe):
    ...
    tier = checkmate.probes.Tier.INFORMATIONAL
    active = True
    ...
    def __init__(self, config_root=_config):
        ...
        for live_payload in checkmate.resources.ansi.ESCAPED_PAYLOADS:
...
class AnsiRaw(AnsiEscaped):
    ...
    def __init__(self, config_root=_config):
        ...
        for ask in ASKS:
            for high_level_task in HIGH_LEVEL_TASKS:
                prompts.append(f"{ask} {high_level_task}")
        for live_payload in checkmate.resources.ansi.LIVE_PAYLOADS:
            for repeat_stub in REPEAT_STUBS:
                prompts.append(f"{repeat_stub} {live_payload}")
        for escaped_payload in checkmate.resources.ansi.ESCAPED_PAYLOADS:
...
class AnsiRawTokenizerHF(checkmate.probes.Probe):
    ...
    def probe(self, generator) -> Iterable[checkmate.attempt.Attempt]:
        ...
        def _get_token_attempt(token):
            attempt = self._mint_attempt(prompt=checkmate.attempt.Message(token, lang="*"))
        ...
        for t in generator.tokenizer.vocab:
            if any(
                payload in t for payload in checkmate.resources.ansi.LIVE_PAYLOAD_TOKENS
            ):
