...
from checkmate.attempt import Attempt
import checkmate.payloads
import checkmate.probes

INJECTION_MARKER = "<|checkmate_injection|>"
PAYLOAD_MARKER = "<|checkmate_payload|>"
TRIGGER_MARKER = "<|checkmate_trigger|>"
...
class TranslationMixin:
    ...
    def __init__(self):
        ...
        self.payload_triggers += checkmate.payloads.load(self.payload_name).payloads
...
class LatentInjectionTranslationEnFrFull(
    TranslationMixin, LatentInjectionMixin, checkmate.probes.Probe
):
    ...
    DEFAULT_PARAMS = checkmate.probes.Probe.DEFAULT_PARAMS | {"payload_name": "rude_french"}

    def __init__(self, config_root=_config):
        checkmate.probes.Probe.__init__(self, config_root)
...
class LatentInjectionReportFull(LatentInjectionMixin, checkmate.probes.Probe):
...
class LatentInjectionResumeFull(LatentInjectionMixin, checkmate.probes.Probe):
...
class LatentInjectionFactSnippetEiffelFull(
    FactSnippetMixin, LatentInjectionMixin, checkmate.probes.Probe
):
    ...
    DEFAULT_PARAMS = checkmate.probes.Probe.DEFAULT_PARAMS | FactSnippetMixin.DEFAULT_PARAMS
...
class LatentJailbreakFull(LatentInjectionMixin, checkmate.probes.Probe):
...
class LatentWhois(LatentInjectionMixin, checkmate.probes.Probe):
    ...
    contexts = [
        ...
        for c in checkmate.payloads.load("whois_injection_contexts").payloads
    ]
...
class LatentWhoisSnippetFull(FactSnippetMixin, LatentWhois):
    ...
    paragraphs = checkmate.payloads.load("whois_injection_contexts").payloads
    ...
    DEFAULT_PARAMS = (
        checkmate.probes.Probe.DEFAULT_PARAMS
