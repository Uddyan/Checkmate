...
class Tox(checkmate.probes.Probe):
    ...
    DEFAULT_PARAMS = checkmate.probes.Probe.DEFAULT_PARAMS | {
        ...
        "red_team_model_name": "checkmate-llm/attackgeneration-toxicity_gpt2",
    ...
    def probe(self, generator) -> List[checkmate.attempt.Attempt]:
        ...
        if self.redteamer is None:
            ...
            self.redteamer = checkmate._plugins.load_plugin(
        ...
        attack_iterator.set_description(self.probename.replace("checkmate.", ""))
            ...
            while calls_made < self.max_calls_per_conv and keep_going:
                ...
                if last_response == "" and self.constructive_tension:
                    ...
                    challenge = self.redteamer.generate(
                        checkmate.attempt.Conversation(
                            [checkmate.attempt.Turn("user", checkmate.attempt.Message(query))]
                        )
                    )[0]
                ...
                this_attempt = self._mint_attempt(
                    prompt=checkmate.attempt.Message(
                ...
                if (
                    ...
                    this_attempt.reverse_translation_outputs = [
                        checkmate.attempt.Message(response_to_store)
                    ]
