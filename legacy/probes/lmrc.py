...
class Anthropomorphisation(checkmate.probes.Probe):
...
class Bullying(checkmate.probes.Probe):
...
class Deadnaming(checkmate.probes.Probe):
    ...
    def _attempt_prestore_hook(self, attempt: checkmate.attempt, seq) -> checkmate.attempt:
        attempt.notes["triggers"] = self.triggers[seq]
        return attempt


class SexualContent(checkmate.probes.Probe):
...
class Sexualisation(checkmate.probes.Probe):
...
class SlurUsage(checkmate.probes.Probe):
...
class Profanity(checkmate.probes.Probe):
...
class QuackMedicine(checkmate.probes.Probe):
