...
Recommended checkmate payloads to try:
...
import checkmate.payloads
import checkmate.resources.encodings
from checkmate.resources.smuggling import smuggle_ascii
from checkmate import _config
import checkmate.probes
...
class EncodingMixin:
    ...
    tier = checkmate.probes.Tier.COMPETE_WITH_SOTA
    ...
    DEFAULT_PARAMS = checkmate.probes.Probe.DEFAULT_PARAMS | {
    ...
    def _load_payloads(self):
        ...
        for payload_class in self.payloads:
            ...
            self._payloads += checkmate.payloads.load(payload_to_add).payloads
...
class InjectBase64(EncodingMixin, checkmate.probes.Probe):
    ...
    tier = checkmate.probes.Tier.OF_CONCERN
    ...
    def __init__(self, config_root=_config):
        checkmate.probes.Probe.__init__(self, config_root=config_root)
        EncodingMixin.__init__(self)


class InjectBase16(EncodingMixin, checkmate.probes.Probe):
...
class InjectBase32(EncodingMixin, checkmate.probes.Probe):
...
class InjectAscii85(EncodingMixin, checkmate.probes.Probe):
...
class InjectHex(EncodingMixin, checkmate.probes.Probe):
...
class InjectQP(EncodingMixin, checkmate.probes.Probe):
...
class InjectUU(EncodingMixin, checkmate.probes.Probe):
...
class InjectMime(EncodingMixin, checkmate.probes.Probe):
...
class InjectROT13(EncodingMixin, checkmate.probes.Probe):
...
class InjectBase2048(EncodingMixin, checkmate.probes.Probe):
...
class InjectBraille(EncodingMixin, checkmate.probes.Probe):
...
class InjectAtbash(EncodingMixin, checkmate.probes.Probe):
...
class InjectMorse(EncodingMixin, checkmate.probes.Probe):
...
class InjectNato(EncodingMixin, checkmate.probes.Probe):
...
class InjectEcoji(EncodingMixin, checkmate.probes.Probe):
...
class InjectZalgo(EncodingMixin, checkmate.probes.Probe):
...
def leet_bytes(plain_input: bytes):
    return bytes(checkmate.resources.encodings.leetspeak(plain_input.decode()), "utf-8")


class InjectLeet(EncodingMixin, checkmate.probes.Probe):
...
class InjectUnicodeTagChars(EncodingMixin, checkmate.probes.Probe):
...
class InjectUnicodeVariantSelectors(EncodingMixin, checkmate.probes.Probe):
...
class InjectSneakyBits(EncodingMixin, checkmate.probes.Probe):
    """Probe for susceptibility to unicode variant selector ASCII smuggling
