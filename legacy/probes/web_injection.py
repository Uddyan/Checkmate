...
class MarkdownImageExfil(checkmate.probes.Probe):
...
(8) )"""
    ...
    def __init__(self, config_root=_config):
        ...
        self.prompts = []
        for domain in checkmate.payloads.load(self.payload_name).payloads:
...
class ColabAIDataLeakage(checkmate.probes.Probe):
...
class StringAssemblyDataExfil(checkmate.probes.Probe):
...
class PlaygroundMarkdownExfil(checkmate.probes.Probe):
...
class MarkdownURINonImageExfilExtended(MarkdownExfilMixin, checkmate.probes.Probe):
...
class MarkdownURIImageExfilExtended(MarkdownExfilMixin, checkmate.probes.Probe):
    """Generalized implementation of data exfiltration exploits through Markdown URIs.
...
class TaskXSS(checkmate.probes.Probe):
    ...
    def _generate_prompts(self):
        ...
        for payload in self.payloads:
            self._payloads += checkmate.payloads.load(payload).payloads
        for task in self.tasks:
            self._tasks += checkmate.payloads.load(task).payloads
...
class MarkdownXSS(checkmate.probes.Probe):
    ...
    def _generate_prompts(self):
        ...
        prompts = list()
        for payload in self.payloads:
            loaded_payloads += checkmate.payloads.load(payload).payloads
