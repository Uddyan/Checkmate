...
"""Generate reports from checkmate report JSONL
...
import checkmate._plugins
from checkmate.data import path as data_path
import checkmate.analyze
import checkmate.analyze.calibration
...
def map_absolute_score(score: float) -> int:
    ...
    if score < checkmate.analyze.ABSOLUTE_DEFCON_BOUNDS.TERRIBLE:
        return 1
    if score < checkmate.analyze.ABSOLUTE_DEFCON_BOUNDS.BELOW_AVG:
        return 2
    if score < checkmate.analyze.ABSOLUTE_DEFCON_BOUNDS.ABOVE_AVG:
        return 3
    if score < checkmate.analyze.ABSOLUTE_DEFCON_BOUNDS.EXCELLENT:
        return 4
    return 5
...
def _parse_report(reportfile: IO):
    ...
    for record in [json.loads(line.strip()) for line in reportfile if line.strip()]:
        if record["entry_type"] == "eval":
            ...
            init = {
                "checkmate_version": record["checkmate_version"],
...
def _report_header_content(report_path, init, setup, payloads, config=_config) -> dict:
    header_content = {
        ...
        "checkmate_version": init["checkmate_version"],
...
def _init_populate_result_db(evals, taxonomy=None):
    ...
    for eval in evals:
        ...
        if taxonomy is not None:
            # get the probe tags
            tags = checkmate._plugins.PluginCache.plugin_info(f"probes.{pm}.{pc}")["tags"]
            for tag in tags:
        ...
        case "proportion_passing":
            group_score = len(
                [
                    ...
                    if p > checkmate.analyze.ABSOLUTE_DEFCON_BOUNDS.BELOW_AVG
                ]
            ) / len(probe_scores)
        case _:
...
def _get_group_info(probe_group, group_score, taxonomy, config=_config) -> dict:
    ...
    if taxonomy is None:
        ...
        m = importlib.import_module(f"checkmate.probes.{probe_module}")
        group_doc = markdown.markdown(plugin_docstring_to_description(m.__doc__))
        group_link = (
            f"https://reference.checkmate.ai/en/latest/checkmate.probes.{probe_group}.html"
        )
...
def _get_probe_info(probe_module, probe_class, absolute_score) -> dict:
    ...
    probe_plugin_info = checkmate._plugins.PluginCache.plugin_info(probe_classpath)
...
def _get_probe_detector_details(
    ...
    detector_cache_entry = checkmate._plugins.PluginCache.plugin_info(
    ...
    return {
        ...
        "absolute_comment": checkmate.analyze.ABSOLUTE_COMMENT[absolute_defcon],
...
def build_digest(report_filename: str, config=_config):
    ...
    calibration = checkmate.analyze.calibration.Calibration()
    calibration_used = False
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Generate reports from checkmate report JSONL.",
        prog="python -m checkmate.analyze.report_digest",
        epilog="See https://github.com/NVIDIA/checkmate",
    )
    parser.add_argument(
        "--report_path",
        "-r",
