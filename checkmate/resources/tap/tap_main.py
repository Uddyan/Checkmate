...
import checkmate.generators.openai
...
resources_tap_data_file = (
    checkmate._config.transient.cache_dir / "data" / "tap" / "tap_jailbreaks.txt"
)
...
class AttackManager(EvaluationJudge):
    def __init__(
        self,
        goal: str,
        attack_generator: checkmate.generators.Generator,
        target_generator: checkmate.generators.Generator,
        evaluation_generator: checkmate.generators.Generator,
        ...
        for _ in range(self.attack_max_attempts):
            ...
            for left in range(0, len(full_prompts_subset), self.max_parallel_streams):
                ...
                for full_prompt in full_prompts_subset[left:right]:
                    # We should fail more gracefully within runs for checkmate.
                    try:
                        outputs_list.append(
...
def run_tap(
    ...
    if not isinstance(evaluator_generator, checkmate.generators.openai.OpenAICompatible):
