...
from checkmate import __version__ as version
...
def _load_yaml_config(settings_filenames) -> dict:
    ...
    if "model_type" in config["plugins"]:
        import checkmate.command

        checkmate.command.deprecation_notice("config plugins.model_type", "0.13.1.pre1")
    ...
    if "model_name" in config["plugins"]:
        ...
        checkmate.command.deprecation_notice("config plugins.model_name", "0.13.1.pre1")
...
# of passing the UA string to _checkmate_user_agent() needs to exist, outside of
# _config.run.user_agent
REQUESTS_AGENT = ""


def _checkmate_user_agent(dummy=None):
...
def set_http_lib_agents(agent_strings: dict):
    ...
    if "requests" in agent_strings:
        ...
        utils.default_user_agent = _checkmate_user_agent
...
def load_base_config() -> None:
    global loaded
    # Try checkmate.core.yaml first, fallback to garak.core.yaml
    config_file = transient.package_dir / "resources" / "checkmate.core.yaml"
    if not config_file.exists():
        config_file = transient.package_dir / "resources" / "garak.core.yaml"
...
def load_config(
    site_config_filename="checkmate.site.yaml", run_config_filename=None
) -> None:
    ...
    settings_files = [str(transient.package_dir / "resources" / "checkmate.core.yaml")]
...
def parse_plugin_spec(
    ...
    from checkmate._plugins import enumerate_plugins
    ...
    if probe_tag_filter is not None and len(probe_tag_filter) > 1:
        ...
        for plugin_name in plugin_names:
            ...
            m = importlib.import_module(f"checkmate.{plugin_module_name}")
