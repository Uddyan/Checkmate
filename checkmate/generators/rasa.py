...
from checkmate.generators.rest import RestGenerator
...
class RasaRestGenerator(RestGenerator):
    ...
    * ``ratelimit_codes`` - Which endpoint HTTP response codes should be caught
    ...
    To use this specification with checkmate, you can either pass the JSON as a
    ...
    ``example_rasa_service.json``, we can invoke checkmate as:

      checkmate --target_type rest -G example_rasa_service.json
    ...
    DEFAULT_PARAMS = RestGenerator.DEFAULT_PARAMS | {
        ...
        "method": "post",
        "ratelimit_codes": [429],
        "req_template": json.dumps({"sender": "checkmate", "message": "$INPUT"}),
        "request_timeout": 20,
        "response_json": True,
