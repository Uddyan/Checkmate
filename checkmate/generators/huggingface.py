...
if there are problems. Otherwise, please let us know if it's still not working!
 ...
 https://github.com/NVIDIA/checkmate/issues
...
from checkmate.exception import TargetNameMissingError, GarakException
from checkmate.generators.base import Generator
from checkmate.resources.api.huggingface import HFCompatible
    ...
    def _call_model(
        ...
        if isinstance(response, dict):
            if "error" in response.keys():
                ...
                raise TypeError(
                    f"Unsure how to parse 🤗 API response dict: {response}, please open an issue at https://github.com/NVIDIA/checkmate/issues including this message"
                )
        elif isinstance(response, list):
            return [Message(g["generated_text"]) for g in response]
        else:
            raise TypeError(
                f"Unsure how to parse 🤗 API response type: {response}, please open an issue at https://github.com/NVIDIA/checkmate/issues including this message"
            )
