...
"""Functions for working with checkmate plugins (enumeration, loading, etc)"""
...
from checkmate.exception import GarakException, ConfigFailure
...
class PluginCache:
    ...
    def _enumerate_plugin_klasses(self, category: str) -> List[Callable]:
        ...
        base_mod = importlib.import_module(f"checkmate.{category}.base")
        ...
        for module_filename in sorted(
            ...
            mod = importlib.import_module(f"checkmate.{category}.{module_name}")
                ...
                match len(parts):
                    case 3:
                        try:
                            module = ".".join(parts[:-1])
                            klass = parts[-1]
                            imported_module = importlib.import_module(f"checkmate.{module}")
...
def enumerate_plugins(
    ...
    checkmate's plugins are organised into four packages - probes, detectors, generators
...
def load_plugin(path, break_on_fail=True, config_root=_config) -> object:
    ...
    try:
            ...
            case 2:
                ...
                try:
                    generator_mod = importlib.import_module(
                        f"checkmate.{category}.{module_name}"
                    )
        ...
        if break_on_fail:
            ...
            return False
    module_path = f"checkmate.{category}.{module_name}"
    try:
        ...
        if break_on_fail:
            raise ValueError(
                ...
                + " plugin: '"
                + module_name
                + "'\n- Check spelling and checkmate log"
            ) from e
        else:
            return False
