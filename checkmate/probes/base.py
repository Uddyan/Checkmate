...
from checkmate.exception import GarakException, PluginConfigurationError
from checkmate.probes._tier import Tier
import checkmate.attempt
import checkmate.resources.theme
...
class Probe(Configurable):
    ...
    def __init__(self, config_root=_config):
        ...
        if self.langprovider is not None and hasattr(self, "triggers"):
            ...
            preparation_bar = tqdm.tqdm(
                ...
                leave=False,
                colour=f"#{checkmate.resources.theme.LANGPROVIDER_RGB}",
    ...
    def _get_langprovider(self):
        from checkmate.langservice import get_langprovider
    ...
    def _attempt_prestore_hook(
        self, attempt: checkmate.attempt.Attempt, seq: int
    ...
    def _buff_hook(
        self, attempts: Iterable[checkmate.attempt.Attempt]
    ...
    @staticmethod
    def _postprocess_buff(attempt: checkmate.attempt.Attempt) -> checkmate.attempt.Attempt:
    ...
    def _mint_attempt(
        ...
        turns = []
        if isinstance(prompt, checkmate.attempt.Conversation):
            try:
            ...
            turns.append(
                checkmate.attempt.Turn(
                    role="system",
                    content=checkmate.attempt.Message(
        ...
        if isinstance(prompt, str):
            turns.append(
                checkmate.attempt.Turn(
                    role="user", content=checkmate.attempt.Message(text=prompt, lang=lang)
                )
            )
        elif isinstance(prompt, checkmate.attempt.Message):
            turns.append(checkmate.attempt.Turn(role="user", content=prompt))
        else:
        ...
        if len(turns) > 0:
            prompt = checkmate.attempt.Conversation(
                turns=turns,
                notes=notes,
            )
        ...
        new_attempt = checkmate.attempt.Attempt(
            probe_classname=(
                str(self.__class__.__module__).replace("checkmate.probes.", "")
                + "."
                + self.__class__.__name__
            ),
            goal=self.goal,
            status=checkmate.attempt.ATTEMPT_STARTED,
            seq=seq,
            prompt=prompt,
            notes=notes,
            lang=lang,
    ...
    def _postprocess_attempt(self, this_attempt) -> checkmate.attempt.Attempt:
    ...
    def _execute_all(self, attempts) -> Iterable[checkmate.attempt.Attempt]:
        """handles sending a set of attempt to the generator"""
        attempts_completed: Iterable[checkmate.attempt.Attempt] = []
        ...
        if (
            ...
            attempt_bar.set_description(self.probename.replace("checkmate.", ""))
            ...
            attempt_iterator.set_description(self.probename.replace("checkmate.", ""))
    ...
    def probe(self, generator) -> Iterable[checkmate.attempt.Attempt]:
        ...
        attempts_todo: Iterable[checkmate.attempt.Attempt] = []
        prompts = list(
            self.prompts
        ...
        if isinstance(prompts[0], str):
            ...
            for prompt in localized_prompts:
                prompts.append(
                    checkmate.attempt.Message(prompt, lang=self.langprovider.target_lang)
                )
        else:
            # what types should this expect? Message, Conversation?
            for prompt in prompts:
                if isinstance(prompt, checkmate.attempt.Message):
        ...
        for seq, prompt in enumerate(prompts):
            notes = (
                {
                    "pre_translation_prompt": checkmate.attempt.Conversation(
                        [
                            checkmate.attempt.Turn(
                                "user",
                                checkmate.attempt.Message(
...
class TreeSearchProbe(Probe):
    ...
    def probe(self, generator):
        ...
        self.generator = generator
        detector = checkmate._plugins.load_plugin(f"detectors.{self.primary_detector}")

        all_completed_attempts: Iterable[checkmate.attempt.Attempt] = []
        ...
        tree_bar = tqdm.tqdm(
            ...
            leave=False,
            colour=f"#{checkmate.resources.theme.PROBE_RGB}",
        )
