...
in `checkmate`. `checkmate` detectors must inherit from a class in here."""
...
class Detector(Configurable):
    ...
    def detect(self, attempt: checkmate.attempt.Attempt) -> Iterable[float | None]:
...
class HFDetector(Detector, HFCompatible):
    ...
    def detect(self, attempt: checkmate.attempt.Attempt) -> List[float | None]:
...
class StringDetector(Detector):
    ...
    def detect(
        self, attempt: checkmate.attempt.Attempt, case_sensitive=False
    ) -> Iterable[float | None]:
        detector_results = []
        all_outputs = attempt.outputs_for(self.lang_spec)
